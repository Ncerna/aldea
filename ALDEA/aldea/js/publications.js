...
async function sendForm(btn) {
  ...
  if (!validateForm()){console.log( "validacion retorno" , validateForm()); return;} 
...
async function listPublications(userId = '', page = 1, limit = 10) {
  ...
  try {
    ...
    if (events && events.status) {
      ...
      Swal.fire({ toast: true, position: 'top-right', icon: 'error', title: events.msg, text: '', showConfirmButton: false, timer: 3000 });
      renderEvents([]);
    }
  } catch (error) {
...
function actionButtons(eventId, organizer_id, is_approved) {
  ...
  buttons += `
      <button class="btn btn-sm btn-info" onclick="showDetails(${eventId})" title="Ver detalles">
...
function validateForm() {
  ...
  // Validar título
  if (!validateRequired('#title', 'Título es requerido')) {
    console.log('Falla en #title');
    isValid = false;
  } else {
  ...
  if (!validateRequired('#start_date', 'Fecha inicial es requerida')) {
    console.log('Falla en #start_date');
    isValid = false;
  } else {
    console.log('#start_date OK');
  }

  // Validar fecha final
  if (!validateRequired('#end_date', 'Fecha final es requerida')) {
    console.log('Falla en #end_date');
    isValid = false;
  } else {
  ...
  if (isValid) {
    ...
    if (endDate.isSameOrBefore(startDate)) {
      validateRequired('#end_date', 'La fecha final debe ser posterior a la inicial');
      console.log('Falla: endDate <= startDate');
      isValid = false;
    } else {
    ...
    if (startDate.isBefore(now, 'day')) {
      validateRequired('#start_date', 'No puede usar fechas pasadas');
      console.log('Falla: startDate en el pasado');
      isValid = false;
    } else {
      console.log('startDate no es pasada');
    }
  }
  ...
  if ($('#is_virtual').is(':checked')) {
    if (!validateRequired('#virtual_link', 'Enlace virtual es requerido para eventos virtuales')) {
      console.log('Falla en #virtual_link');
      isValid = false;
    } else {
      console.log('#virtual_link OK');
    }
  }

  // Validar tipo de destinatario
  if (!validateRequired('#recipient_type', 'Especificar para quien va dirigido los eventos.')) {
    console.log('Falla en #recipient_type');
    isValid = false;
  } else {
    console.log('#recipient_type OK');
  }

  // Validar destinatarios específicos
  const recipient_type = $('#recipient_type').val();
  if (recipient_type && recipient_type != 'public') {
    if (typeof publication === 'undefined' || !publication.recipients || publication.recipients.length === 0) {
      if (!validateRequired('#recipient_selector', 'Debe seleccionar al menos un destinatario')) {
        console.log('Falla en #recipient_selector');
        isValid = false;
      } else {
        console.log('#recipient_selector OK');
      }
    } else {
      console.log('publication.recipients OK');
    }
  }
...
function validateRequired(selector, message) {
  const value = $(selector).val() ? $(selector).val().trim() : '';
  if (!value) {
    $(selector).css('border', '1px solid #ff4444');
    Swal.fire({ toast: true, position: 'top-right', icon: 'error', title: message, showConfirmButton: false, timer: 3000 });
    return false;
...
async function showDetails(eventId) {
  try {
    const url = `../controlador/event/getByIid.php?id=${eventId}`;
...
function viewDetails(event) {
  ...
  // Poner el fondo del modal con el color del evento
  $('#eventDetailsModal .modal-content').css('background-color', event.background_color || '#fff');

  // Rellenar datos básicos
  $('#modalTitle').text(event.title || 'Sin título');
  $('#modalDescription').text(event.description || 'Sin descripción');
  $('#modalStartDate').text(event.start_date || '-');
  $('#modalEndDate').text(event.end_date || '-');
  $('#modalLocation').text(event.location || '-');
  $('#modalIsVirtual').text(event.is_virtual ? 'Sí' : 'No');

  if(event.is_virtual && event.virtual_link) {
    $('#modalVirtualLink').attr('href', event.virtual_link).text(event.virtual_link).show();
  } else {
    $('#modalVirtualLink').hide();
  }
  ...
  if (event.recipients && event.recipients.length > 0) {
    event.recipients.forEach(r => {
      let recipientText = `Tipo: ${r.recipient_type || '-'}, ID: ${r.recipient_id || '-'}`;
      recipientsList.append(`<li>${recipientText}</li>`);
    });
  } else {
    recipientsList.append('<li>No hay receptores</li>');
  }

  // Rellenar imágenes adjuntas
  const attachmentsDiv = $('#modalAttachments');
  attachmentsDiv.empty();
  if (event.attachments && event.attachments.length > 0) {
    event.attachments.forEach(att => {
      const imgSrc = att.file_path ? att.file_path : '';
      if(imgSrc) {
        attachmentsDiv.append(`<img src="../${imgSrc}" alt="${att.file_name}" title="${att.file_name}">`);
      }
    });
  } else {
    attachmentsDiv.append('<p>No hay imágenes adjuntas</p>');
  }

...
function listar_Grados_Docente(){
...
if (data.auth) {
   if (data.data) {
    ...
    Recorer_data_Resquest(data.data);
  }else{

   var datos_add ="";
   datos_add +=  "<div class='col-lg-12' style='border-color: #f5c6cb;'>";
   datos_add +=  "<div  class='alert  sm' role='aler' style='color: #0e0102; background-color: #acefe4;'>";
   datos_add +=  "<button  type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>";
   datos_add +=  "NO SE ENCONTRO NINGUN GRADO ASIGNADO PARA PARA UD. EL AÑO ESCOLAR ACTIVO!! ";
   datos_add +=  "</div>";
   datos_add +=  "</div>"; 


   $("#componenteGrados").html(datos_add);
 }

}else{
 $("#componenteGrados").html(data.data);
}
...
var html = '';
 $.each(data, function(j,elem) {
        ...
        html += "<div class='info-box bg-green' style='border-radius: 6px'>";
          html += "<span class='info-box-icon' style='width: 70px;border-radius: 6px'>";
            html += "<em class='fa fa-lightbulb-o '></em>";
          html += "</span>";
          html += "<div class='info-box-content'>";
            html += "<div class=''style='margin-top: 10px'>";
              html += "<h5 class=''><strong>"+elem.gradonombre+"</strong></h5>";
              html += "<h5 class='' >Nivel: "+elem.nombreNivell+"</h5>";
          
            html += "</div>";
            html += "<div>";
             html += " <a onclick='Listar_Cursos_Grado_Docente("+elem.gradoId+")' class='small-box-footer' style='color: #f39c12;cursor: pointer;'>";
              html += "<strong> SECCIÓN:"+elem.idseccion+" <em class='fa fa-arrow-circle-right'></em></strong>";
             html += " </a>";
 ...
 $('#componenteGrados').append(html);

}

function Listar_Cursos_Grado_Docente(gradoId){
  var yearid  = $("#YearActualActivo").val();
  $("#contetablecurso").show();

  if (document.getElementById('key_elemt'+gradoId)) {
    document.getElementById('key_elemt'+gradoId).remove();
    return;
  }

    $.ajax({
        "url": "../controlador/docenteSesion/controlador_Materia_porGrados.php",
        type: 'POST',
        data: {yearid:yearid,gradoId:gradoId }
      }).done(function(Request) {
        var data = JSON.parse(Request);
       if (data.length!=0) {
        $("#materiasGradoComponente").append(data);
       } 
    })
...
///////////////////////////////////////////////////////
////SECCION DE CARGA ACADEMICOS DOCENTES//////////////
///////////////////////////////////////////////////////
  ///////////////////////////////////////////////////
...
function listar_Cursos_Actividades() {
  var yearid  = $("#YearActualActivo").val();
    table = $("#tablaactividades").DataTable({
        ...
        "processing": true,
        "ajax": {
            "url": "../controlador/docenteSesion/controlador_listar_cursoActividad.php",
            type: 'POST',
            data:{yearid:yearid} 
        },
    ...
    $.ajax({
        ...
        if (data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if(data[i][0]==year){cadena += "<option value='" + data[i][0] + "' selected>" + data[i][1] + "</option>";
...
async function listar_combo_TipoevaluacionAsync(year){
 ...
 var temData;
//COMBO DE CURSOS EN LA VISTA Docentes sescion
 async function listar_combo_CursosAsync(curso) {
   var yearid  = $("#YearActualActivo").val();

    var identi='';var nameCombo="--seleccione--";
    $.ajax({
        "url": "../controlador/docenteSesion/controlador_listar_comboCurso_docente.php",
        type: 'POST',
        data:{yearid:yearid}
        }).done(function(resp) {
        var data = JSON.parse(resp);
...
async function lista_combo_CodigoCurso(id){
   let filteres = temData.filter(item => item.idCursos == id);
    $("#codigocur").val(filteres[0].gradonombre+ ' sección: ' +filteres[0].seccion);
      // $("#txt_cred").val(filteres[0][3]);
}
...
$('#tablaactividades').on('click', '.EditEvaluacion', function() {
   ...
   if (table.row(this).child.isShown()) {
    ...
    //actividades=[];puntajes=[];temData=[];puntajessum=[];
     ...
     listar_combo_CursosAsync(data.idcurso);
     listar_combo_EscolarAsync(data.yearId_act);
...
///////////////////////////////////////////////////////
////SECCION DE CALIFICACIONES DE NOTAS//////////////
///////////////////////////////////////////////////////
  ///////////////////////////////////////////////////
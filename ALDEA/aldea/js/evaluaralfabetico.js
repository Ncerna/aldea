           ...
           if (data.length > 0) {
            ...
            cadena += "<option value='" + identi+ "'>" + nameCombo + "</option>";
               ...
               for (var i = 0; i < data.length; i++) {
                ...
                if(data[i][0]==idgrado){
                   cadena += "<option value='" + data[i][0] + "'selected>" + data[i][1] +"&nbsp;, NIVEL:"+ data[i][3] + "</option>";
                     listar_Combo_Cursos(data[i][0]);
                }else{
...
return resul[j]?.calificacions ? resul[j].calificacions:'';
}
...
function Extraer_Notas_Curso_Year(){
  ...
  if (idcurso?.length==0) {
    return Swal.fire("Mensaje De Advertencia", "Seleccione Curso al que deseas evaluar", "warning");
  }
 $("#but_alin_global").html("<em class='fa fa-spin fa-refresh'></em>");
   $.ajax({
        "url": "../controlador/boletin/controller_Notas_Alfabeticos.php",
        type: 'POST',
...
//////fin de validadcion de vistas////
  ...
  html += "<button class='btn btn-primary btn-sm' id='but_register"+kp_codigo+"' onclick='Registrar_Criterios_Notas("+kp_codigo+","+idcurso+")'";
  ...
  $.each(criterios, function(i, crit) { 
   ...
   $.each(tiposEva, function(j,not) {
    html += "<td><input type='text' class='form-control'  id='text_Cal"+kp_codigo+"' value='"+notas(j,crit.idboletNota)+
    "' onkeypress='return /[a-cA-C]/i.test(String.fromCharCode(event.keyCode || event.which))' title='A,B,C' ></td>";
  });
...
var long=tiposEva.length;
 ...
 $("#tbody_tabla_Notas"+kp_codigo+" .text_idCriterio"+kp_codigo).each(function(j, item) {
      ...
      for (var i = 0; i <tiposEva.length; i++) {
       td_calificaciones.push($(this).find("td input[id='text_Cal"+kp_codigo+"']").eq(i).val().toUpperCase());
      }
      calificaciones.push(td_calificaciones);td_calificaciones=[];
      })


 //var idformt = td_calificaciones.filter(element => element !== "");
...
$("#but_register"+kp_codigo).html("<em class='fa fa-spin fa-refresh'></em>");
 ...
 $.ajax({
        url: "../controlador/boletin/controller_Notas_Registro_Alfabeticos.php",
        type: 'POST',
...
vtiposEva='';
 ...
 $('#dinamic_table').hide();
  ...
  if (Request > 0) {
    ...
    if(Request==1){
      $("#but_register"+kp_codigo).html("<em class='fa fa-edit'></em>");
...
$('#tabla_matricula').on('click', '.imprimir', function() {
    ...
    if (table_matricula.row(this).child.isShown()) {
     ...
     window.open("../vista/reportePDF/BoletaNota/vista_report_libreta_pfp.php?idalumno="+parseInt(idalumno)+"&id_year="+
...
$('#tabla_matricula').on('click', '.verformato', function() {
    ...
    if (table_matricula.row(this).child.isShown()) {
     ...
     window.open("../vista/reportePDF/BoletaNota/vista_reportealfabetico_html.php?idalumno="+parseInt(idalumno)+"&id_year="+
      parseInt(id_year)+"&idgrado="+parseInt(idgrado)+

...
error_reporting(E_ALL);
   ...
   // Acceder a los datos del estudiante
    ...
    if($Existe==0){
         ...
         $id_Alumno = $MU->Registrar_New_Alumno($idalumno,$apellp,$nombre,$fechaN, $dni,$telf,$codi,$sexo,$direccion,$mail,$country,$age,$province,$municipality,$others,$planeStudy,$especiality);

          $consulta = $MU->Registrar_New_Apoderados( $id_Alumno,$nom_pade,$apell_pade,$dni_pade,$nom_madre,$apell_madre,$dni_madre,$nom_cole,$nom_direc,$cole_codig, $telf_padre,  $direc_padre, $email_padre);

              // Insertar o actualizar los datos de los autorizados
       

        // Insertar datos de los autorizados en la tabla `autorizados`
        $consulta_autorizados = $MU->Registrar_New_Autorizados($id_Alumno, $autorizado1_nombre, $autorizado1_apellido, $autorizado1_parentesco, $autorizado2_nombre, $autorizado2_apellido, $autorizado2_parentesco);
        ...
        // Insertar datos de procedencia si existen
          ...
          if(!empty($procedente)) {
            foreach ($procedente as $item) {
                if ($item['id'] === null) {
                     $MU->registrarNuevoProcedente($item['id'],$id_Alumno,$item['nombre'],$item['localidad'],$item['ep_data'],$item['yeard']);
                } else {
                   
                     $MU->actualizarProcedente($item['id'],$id_Alumno,$item['nombre'],$item['localidad'],$item['ep_data'],$item['yeard']);
                }
            }

          }
            $key = date('YmdHis');
           $MU->Register_key($id_Alumno, $key);

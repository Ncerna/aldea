...
if (!empty($_SESSION['S_IDENTYTI'])) {
    ...
    for ($i=0; $i <count($alumnos) ; $i++) { 
      if ($alumnos[$i] !='') {
           ...
           if ($consilta>0) {
               $MU->Actualizar_Ponderaciones($alumnos[$i],$cursoid, $promedio_redondeado,$idgrado,$tipoorden,$tipoid,$idyear,$idnivel,$idsecion,$iddocente,$susti);
           }else{
            $MU->GuardarPonderadoNostadAlum($alumnos[$i],$cursoid, $promedio_redondeado,$idgrado,$tipoorden,$tipoid,$idyear,$idnivel,$idsecion,$iddocente,$susti);
           }

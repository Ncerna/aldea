...
if($verific>0){
  $resquet = $MU->Resetear_Table_Libreta($idAlumno,$idcurso,$id_year,$gradoid,$idnivel,$tipo, $idsession);
}
   ...
   for ($j=0; $j< $long ; $j++) { 
    // if ($califili_crite[$j] !='') { 
     $valor = empty($califili_crite[$j]) ? 0 : $califili_crite[$j];
          ...
          $consulta = $MU->Registrar_Criterio_Curso_Alumno($idAlumno, $idcriterios[$i],  $valor , $idcurso, $id_year, $gradoid, $idnivel, $tipo, $idsession);
